...
namespace ApiTester
{
    public class RequestProvider
    {
        ...
        /// <returns></returns>
        public RequestSetting HandleReqSetting(string uri, string data, Options options, Dictionary<string, string> cookieDic)
        {
            ...
            {
                newData = (JsonConvert.DeserializeObject<ExpandoObject>(data));
            }            

            string crypto = options.crypto;
            if (string.IsNullOrEmpty(crypto))
            {
                crypto = Cfg.config.APP_CONF.Encrypt ? "eapi" : "api";
            }

            switch (crypto)
            {
                case "weapi":
                    headers["Referer"] = Cfg.config.APP_CONF.Domain;
                    ...
                    url = Cfg.config.APP_CONF.Domain + "/weapi/" + uri.Substring(5);
                    break;
                case "linuxapi":
                    headers["User-Agent"] = options.ua ?? ChooseUserAgent("linuxapi", "linux");
                    newData = new ExpandoObject();
                    newData.method = "POST";
                    newData.url = Cfg.config.APP_CONF.Domain + uri;
                    newData.@params = data;
                    //encryptData = JsonConvert.SerializeObject(newData);

                    url = Cfg.config.APP_CONF.Domain + "/api/linux/forward";
                    break;
                case "eapi":
                case "api":
                    ...
                    headers["Cookie"] = Utils.DictionaryToString(cookieDic);
                    ...
                    if (crypto == "eapi")
                    {
                        ...
                        else if (Cfg.config.APP_CONF.EncryptResponse)
                        {
                            newData.E_R = Cfg.config.APP_CONF.EncryptResponse;
                        }
                        //newData.E_R = options.e_r ?? newData.E_R ?? Cfg.config.APP_CONF.EncryptResponse;

                        //options.e_r = newData.E_R;

                        //encryptData = JsonConvert.SerializeObject(CryptoUtils.Eapi(uri, newData));

                        newData = CryptoUtils.Eapi(uri, newData);
                        url = Cfg.config.APP_CONF.Domain + "/eapi/" + uri.Substring(5);
                    }
                    else
                    {
                        url = Cfg.config.APP_CONF.Domain + uri;
                        //encryptData = JsonConvert.SerializeObject(newData);
                    }
        ...
        public static ExpandoObject HandleCrypto(Options options, object data, Dictionary<string, string> headers, Dictionary<string, string> cookie, string uri)
        {
            ...
            if (string.IsNullOrEmpty(crypto))
            {
                if (Cfg.config.APP_CONF.Encrypt)
                {
                    crypto = "eapi";
                }
                else
